...
/**
 * Server-side controller responsible for handling incoming client connections,
 * authenticating them, and routing protocol commands to the correct controller actions.
 * <p>
 * This controller listens for incoming requests via a {@link Socket}, performs user
 * authentication, and continuously processes commands sent by the client until the connection is closed.
 * </p>
 */
public class HandleClientsController {

    /**
     * Handles the full interaction cycle with a connected client.
     * <p>
     * This includes:
     * <ul>
     *     <li>Reading authentication information (email + password).</li>
     *     <li>Verifying the credentials against {@link AuthenticationRepository}.</li>
     *     <li>Sending an "OK" or "FAIL" response based on login success.</li>
     *     <li>Delegating further requests to {@link #menuOptionsToExecute(BufferedReader, PrintWriter, Gson)}.</li>
     * </ul>
     *
    ...
    public static void handleClient(Socket clientSocket) {
        try (BufferedReader in = new BufferedReader(new InputStreamReader(clientSocket.getInputStream()));
            ...
            // Proceed with command execution if authenticated
    ...
    /**
     * Waits for and processes client-side commands, invoking the corresponding server-side actions.
     * <p>
     * The commands processed include:
     * <ul>
     *     <li>{@code FindCustomerOfRepresentative} → retrieves customer's NIF by representative.</li>
     *     <li>{@code FindShow4Customer} → retrieves shows for a customer.</li>
     *     <li>{@code FindCustomersProposals} → retrieves customer's proposals.</li>
     *     <li>{@code AnalyseProposalResponse} → applies representative's decision on a proposal.</li>
     * </ul>
     * Any unrecognized command will trigger an alert message.
     *
     * @param in    BufferedReader to receive client messages.
     * @param out   PrintWriter to send responses.
     * @param gson  Gson instance for JSON (de)serialization.
     */
